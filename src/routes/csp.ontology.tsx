...
export const Route = createFileRoute("/csp/ontology")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Capa 2. Grafo semántico Neo4j-compatible con relaciones DEPENDS_ON · VALIDATED_BY · PROTECTED_BY." },
    ],
  }),
    ...
    <div>
      ...
      <div className="p-6 grid grid-cols-1 xl:grid-cols-4 gap-6">
            ...
            <div className="relative aspect-[4/3] w-full bg-[radial-gradient(circle_at_center,oklch(0.18_0.03_265/0.4),transparent_70%)]">
              <svg viewBox="0 0 720 620" className="absolute inset-0 h-full w-full">
                ...
                {CANON_ENTITIES.map((c) => {
                  ...
                  return (
                    <g key={c.id} onMouseEnter={() => setHovered(c.id)} onMouseLeave={() => setHovered(null)} className="cursor-pointer">
                      <circle cx={p.x} cy={p.y} r={hi ? 11 : 7} fill={fed.accent} fillOpacity={hi ? 1 : 0.85} stroke="oklch(0.12 0.02 265)" strokeWidth={1} />
                      <text x={p.x} y={p.y + 24} textAnchor="middle" fontSize="9" fill="oklch(0.78 0.02 265)" fontFamily="ui-monospace, monospace">
                        {c.canonicalId}
                      </text>
                    </g>
                  );
        ...
        <div className="space-y-4">
          <Panel eyebrow="INSPECTOR" title={hovered ? canonById(hovered)?.canonicalId ?? "—" : "Hover un nodo"}>
            {hovered && canonById(hovered) ? (
              <div className="space-y-2 text-xs">
                <div className="mono text-[10px] text-muted-foreground break-all">{hovered}</div>
                <p className="text-foreground/85 leading-relaxed">{canonById(hovered)!.description}</p>
                <div className="mono text-[10px] text-muted-foreground">edges: {ONTOLOGY_EDGES.filter((e) => e.from === hovered || e.to === hovered).length}</div>
              </div>
            ) : (
              <p className="text-xs text-muted-foreground">Coloca el cursor sobre un nodo para ver sus aristas.</p>
            )}
          </Panel>
            ...
            <ul className="divide-y divide-border">
              {Object.entries(EDGE_COLOR).map(([kind, color]) => (
                <li key={kind} className="px-4 py-2 flex items-center gap-2 text-xs">
                  ...
                  <span className="mono text-[10px] uppercase tracking-wider text-muted-foreground flex-1">{kind}</span>
      ...
      <div className="px-6 pb-8">
        <Panel eyebrow="EDGES · TABLE" title="Aristas canónicas" bodyClassName="p-0">
          <table className="w-full text-xs">
            ...
            <tbody>
              {ONTOLOGY_EDGES.map((e, i) => (
                <tr key={i} className={cn("border-b border-border/60 hover:bg-secondary/30 cursor-pointer", (hovered === e.from || hovered === e.to) && "bg-secondary/60")}
                  onMouseEnter={() => setHovered(e.from)}
                  onMouseLeave={() => setHovered(null)}>
                  <td className="px-4 py-2.5 mono text-[11px] truncate max-w-[260px]">{e.from}</td>
                  <td className="px-4 py-2.5 mono text-[10px] uppercase tracking-wider" style={{ color: EDGE_COLOR[e.kind] }}>{e.kind}</td>
