...
import { Activity, ArrowUpRight, Cpu, Database, Fingerprint, Network, ShieldCheck } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Executive view of the TAMV heptafederated ecosystem: federations, kernels, system health and constitutional observability." },
      { property: "og:title", content: "Kodex Home · TAMV Core" },
      { property: "og:description", content: "Sovereign operational console for the TAMV ecosystem." },
    ],
  }),
  component: Index,
});
    ...
    <div>
      ...
      <div className="px-6 py-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          <Stat label="Repos canonizados" value={KPI.repos} delta="+12 / 24h" hint="Singularity ingest" />
          ...
          <Stat label="Artefactos científicos" value={KPI.artefacts.toLocaleString()} delta="+1.2k / 7d" hint="ORCID · Zenodo · Figshare" />
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <Panel
            ...
            action={
              <Link to="/nexus" className="mono text-[10px] uppercase tracking-wider text-muted-foreground hover:text-foreground">
                EXPLORE →
              </Link>
            }
          ...
          <Panel
            ...
            action={
              <Link to="/events" className="mono text-[10px] uppercase tracking-wider text-muted-foreground hover:text-foreground">
                ALL →
              </Link>
            }
            bodyClassName="p-0"
              ...
              {lastEvents.map((e) => (
                <li key={e.id} className="px-4 py-3 hover:bg-secondary/40 transition-colors">
                  <div className="flex items-center justify-between gap-3">
                    <span className={cn("mono text-[10px] uppercase tracking-wider", {
        ...
        <Panel eyebrow="HEPTAFEDERATED · F1 → F7" title="Federations health overview" bodyClassName="p-0">
            ...
            {FEDERATIONS.map((f) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="mono text-[10px] uppercase tracking-wider" style={{ color: f.accent }}>
                    {f.code}
                  </span>
                ...
                <div className="mt-1 text-[11px] text-muted-foreground leading-snug line-clamp-2">{f.mandate}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Panel eyebrow="KERNELS · CORE" title="Constitutional kernels online">
            <ul className="space-y-3 text-sm">
              {[
                { icon: ShieldCheck, name: "EOCT Constitutional Runtime", meta: "ABAC · trust engine" },
                ...
                return (
                  <li key={k.name} className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mono text-[10px] uppercase tracking-wider text-muted-foreground">{k.meta}</div>
                    </div>
            ...
            <ul className="space-y-2.5">
              {TOMOS.map((t) => (
                <li key={t.id} className="flex items-start gap-3 text-sm">
                  <span className="mono text-[10px] uppercase tracking-wider text-accent w-8 shrink-0 pt-0.5">{t.number}</span>
                  <div className="min-w-0">
                    <div className="truncate">{t.title}</div>
                    <div className="text-[11px] text-muted-foreground leading-snug">{t.summary}</div>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <Panel eyebrow="TIMELINE · ECOSYSTEM" title="Hitos canónicos">
            <ol className="relative border-l border-border pl-5 space-y-4">
              {[
                { t: "2024 Q1", title: "Génesis ADN Anubis", body: "Manifiesto Isabella Villaseñor IA · ICIL definido." },
                { t: "2024 Q3", title: "Singularity v0", body: "Absorción inicial de 78 repos en monorepo Kodex." },
                { t: "2025 Q1", title: "Heptafederación TAMV-K5", body: "Siete federaciones canonizadas y operativas." },
                { t: "2025 Q4", title: "BookPI Ledger", body: "Columna vertebral de eventos y DOI batch." },
                { t: "2026 Q1", title: "EOCT Runtime v1", body: "Constitutional ABAC + Lightning Justice." },
                { t: "2026 Q2", title: "Kodex v1.0", body: "209 repos canonizados · launch institucional." },
              ].map((m, i) => (
                <li key={i} className="relative">
                  <span className="absolute -left-[26px] top-1.5 h-2 w-2 rounded-full bg-primary ring-2 ring-background" />
                  <div className="mono text-[10px] uppercase tracking-wider text-muted-foreground">{m.t}</div>
