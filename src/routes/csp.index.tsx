...
export const Route = createFileRoute("/csp/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Núcleo proto-cognitivo federado: ingesta, canon, grafo ontológico, BookPI y self-assembly." },
...
const LAYERS = [
  { n: "0", title: "Semantic Ingestion Engine", icon: Layers, to: "/csp/ingestion", desc: "Repos, manifests, PDFs, webhooks. Pre-clasificación canónica." },
  { n: "1", title: "Canon Registry Engine", icon: Database, to: "/csp/canon", desc: "Caos → canon. ID, tipo, federación, hash, dependencias." },
  { n: "2", title: "Ontological Graph Engine", icon: GitBranch, to: "/csp/ontology", desc: "Neo4j-compatible: DEPENDS_ON · VALIDATED_BY · PROTECTED_BY." },
  { n: "3", title: "Memory — BookPI Ledger", icon: Network, to: "/events", desc: "Append-only event ledger con traceId + hash + entityId." },
  { n: "4", title: "Self-Assembly Engine", icon: Workflow, to: "/csp/assembly", desc: "Federation Compiler™ → docker-compose ejecutable." },
];
    ...
    <div>
      ...
      <div className="p-6 space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          <Stat label="REPOS INGESTED" value={CSP_KPI.reposIngested} hint="GitHub · Zenodo · manifests" />
          <Stat label="CANON ENTITIES" value={CSP_KPI.canonEntities} delta="+3" hint="canonical=true" />
          <Stat label="ONTOLOGY EDGES" value={CSP_KPI.ontologyEdges} hint="Neo4j-compatible" />
          <Stat label="BOOKPI EVENTS" value={CSP_KPI.bookpiEvents.toLocaleString()} hint="append-only ledger" />
          <Stat label="PENDING CONFLICTS" value={CSP_KPI.pendingConflicts} hint="korima-validator" />
                ...
                <li key={l.n}>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">CAPA {l.n}</div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <Panel eyebrow="CANON · LATEST" title="Entidades canonizadas" bodyClassName="p-0" className="xl:col-span-2">
            <ul className="divide-y divide-border">
              {CANON_ENTITIES.slice(0, 6).map((c) => (
                <li key={c.id} className="px-4 py-3 flex items-center gap-3">
                  <Boxes className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                  <span className="mono text-[10px] uppercase tracking-wider text-muted-foreground truncate w-[260px]">{c.id}</span>
                  <span className="text-xs flex-1 truncate text-foreground/85">{c.description}</span>
              ...
              {BOOKPI_EVENTS.slice(0, 6).map((e) => (
                <li key={e.id} className="px-4 py-2.5 text-xs">
                  <div className="flex items-center gap-3">
                    <span className="mono text-[10px] text-muted-foreground tabular w-16">{e.ts}</span>
                    <span className="mono text-[10px] uppercase tracking-wider text-accent w-24">{e.eventType}</span>
                  </div>
                  <div className="mono text-[10px] text-muted-foreground truncate mt-0.5">{e.entityId}</div>
                </li>
              ))}
            </ul>
          </Panel>
        ...
        <Panel eyebrow="FEDERATIONS · COVERAGE" title="Distribución canónica por federación" bodyClassName="p-0">
          <ul className="divide-y divide-border">
            {FEDERATIONS.map((f) => {
              ...
              return (
                <li key={f.id} className="px-4 py-3 flex items-center gap-4">
                  ...
                  <span className="mono text-[10px] text-muted-foreground tabular w-10 text-right">{n}</span>
                </li>
              );
            })}
          </ul>
